...
rich_train_df <- rich_train_df[rich_train_df$record_type < 1, ]
table(rich_train_df$next_record)

write.csv(rich_train_df, "rich_train_1.csv")



train <- read.csv("rich_train_1.csv")
str(train)
train <- train[train$next_record == 1, ]
...
train_risk_knn <- train_risk[, c("homeowner", "location", "group_size", "car_age", "age_oldest", "age_youngest", "married_couple")]
test_risk_knn <- test_risk[, c("homeowner", "location", "group_size", "car_age", "age_oldest", "age_youngest", "married_couple")]
...
train_rf_t <- train_rf[  , c( "day", "group_size", "homeowner", "car_age", "risk_factor", "married_couple", "location", "age_youngest", "age_oldest", "C_previous", "A", "B", "C", "D", "E", "F", "G", "cost")]


A_rfmod <- randomForest(y = as.factor(train_rf$next_A), x = train_rf_t, ntree = 100, importance = TRUE)

B_rfmod <- randomForest(y = as.factor(train_rf$next_B), x = train_rf_t, ntree = 100, importance = TRUE)
C_rfmod <- randomForest(y = as.factor(train_rf$next_C), x = train_rf_t, ntree = 100, importance = TRUE)
D_rfmod <- randomForest(y = as.factor(train_rf$next_D), x = train_rf_t, ntree = 100, importance = TRUE)
E_rfmod <- randomForest(y = as.factor(train_rf$next_E), x = train_rf_t, ntree = 100, importance = TRUE)
F_rfmod <- randomForest(y = as.factor(train_rf$next_F), x = train_rf_t, ntree = 100, importance = TRUE)
G_rfmod <- randomForest(y = as.factor(train_rf$next_G), x = train_rf_t, ntree = 100, importance = TRUE)
